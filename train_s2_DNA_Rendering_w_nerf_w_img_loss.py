...
# def load_image(ref_img_path):
#     part_name = str(ref_img_path).split('/')[-4][:-8]
#     smc_name = str(ref_img_path).split('/')[-4][-7:]
#     ref_view_index = int(str(ref_img_path).split('/')[-3][-4:])
#     ref_img_idx = int(str(ref_img_path).split('/')[-1][:-4])
...
#     ratio = 0.375
#     if ratio != 1.:
#         H, W = int(ref_img.shape[0] * ratio), int(ref_img.shape[1] * ratio)
#         ref_img = cv2.resize(ref_img, (W, H), interpolation=cv2.INTER_AREA)
#         # ref_msk = cv2.resize(ref_msk, (W, H), interpolation=cv2.INTER_NEAREST)
#         ref_K[:2] = ref_K[:2] * ratio
...
def main(cfg):
    ...
    if cfg.unet_additional_kwargs.use_crossview_module:
        for name, module in denoising_unet.named_modules():
            if "crossview_modules" in name:
                for params in module.parameters():
                    params.requires_grad = True
    # if cfg.unet_additional_kwargs.use_crossview_3d_attention:
    #     for name, module in denoising_unet.named_modules():
    #         if "attn_view" in name or "norm_view" in name:
    #             for params in module.parameters():
    #                 params.requires_grad = True
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for _, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.validation.validation_steps == 0 or global_step == 1000:
                    if accelerator.is_main_process:
